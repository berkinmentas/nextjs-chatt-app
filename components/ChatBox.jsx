...
const ChatBox = () => {
  ...
  const handleSend = () => {
    if (textBox.length > 0) {
      addDoc(collection(firestore, `chats/${id}/messages`), {
        message: textBox,
        sender: user.email,
  ...
  //      Ekran boyutuna göre scroll ekle
  useEffect(() => {
  ...
  return (
    <section className="bg-gray-700 relative flex flex-col rounded-lg h-full shadow-xl justify-between shadow-gray-800 w-full text-left text-lg text-white ">
      ...
      <div className="flex gap-x-6 items-center border-b-2 border-dPri pb-4 bg-mSec rounded-t-lg p-4">
        {/*         Geri butonu */}
        <button
          className="lg:hidden border-2 border-white h-max flex items-center rounded-full "
          onClick={() => router.back()}
        >
          <FontAwesomeIcon
            icon={faArrowCircleLeft}
            className="text-dPri bg-white rounded-full text-2xl"
          />
        </button>
      ...
      <div className="flex justify-center rounded-b-lg items-center w-full gap-x-4 p-4 self-end justify-self-end border-t-2 border-dPri bg-mSec ">
        ...
        <button onClick={() => handleSend()}>
          <FontAwesomeIcon
            icon={faPaperPlane}
            className="p-3 shadow-lg shadow-gray-800 rounded-lg hover:scale-105 transition-all hover:bg-lPri bg-dPri outline-0"
          />
        </button>
      </div>
    </section>
